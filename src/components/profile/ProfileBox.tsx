...
export const ProfileBox: React.FC<ProfileBoxProps> = ({ profile }) => {
  ...
  return (
    <Box display={['block', 'flex']} justifyContent={['center', 'space-between']}>
      ...
      <Box mt={{ base: 4, md: 0 }}>
        <Flex align="center" direction={['column', 'row']}>
          ...
          {user?.id === profile.id ? null : (
            <Button variant="outline" size="xs" rounded="true" ml={['0', '6']} onClick={() => toggleFollow(profile)}>
        ...
        <Flex align="center" mt={[2, 0]} direction={['column', 'row']}>
          ...
          <Text fontSize="sm" color="gray.500" fontWeight="semibold" ml={['0', '5']}>
